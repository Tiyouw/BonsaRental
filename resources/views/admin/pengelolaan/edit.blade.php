        ...
        <form action="{{ route('pengelolaan.update', $produk->id_produk) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi" id="deskripsi" rows="4"
                          class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>{{ old('deskripsi', $produk->deskripsi) }}</textarea>
            </div>

            <div class="mb-4">
                <label for="harga_per_hari" class="block text-gray-700 text-sm font-bold mb-2">Harga per Hari (Rp):</label>
                <input type="number" name="harga_per_hari" id="harga_per_hari" value="{{ old('harga_per_hari', $produk->harga_per_hari) }}"
            ...
            <div class="mb-4">
                <label for="gambar" class="block text-gray-700 text-sm font-bold mb-2">Gambar Produk (Biarkan kosong jika tidak ingin mengubah):</label>
                ...
                @if($produk->gambar)
                    <div class="mt-2">
                        ...
                        <img src="{{ asset('storage/' . $produk->gambar) }}" alt="{{ $produk->nama_produk }}" class="w-32 h-auto object-cover rounded">
                    </div>
                @endif
            </div>
